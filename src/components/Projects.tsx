...
function Projects() {
    ...
    let items: any = localStorage.getItem("projects")

    let project = JSON.parse(items)
    ...
    return (
        <div className=" tablet:w-[100%] phone:w-[70vw] phone:h-[87vh] tablet:h-[87vh] max-w-[800px] w-[100vw] flex flex-wrap justify-between overflow-y-scroll mt-[20px] gap-[30px] h-[550px] ">
            {projects.map((item: any) => {
                return (
                    <div key={item.id} className=" w-[220px] tablet:w-[290px] tablet:h-[520px] rounded-[20px] bg-slate-900 h-[400px] ">
                        ...
                        <div className=" flex justify-center mt-[10px] tablet:mt-[20px] gap-[20px] ">
                            <button onClick={() => handleClick(item.id)} className=" tablet:w-[50px] tablet:h-[50px] tablet:text-[25px] tablet:rounded-[20px] w-[30px] h-[30px] bg-[red] rounded-[5px] fas fa-heart "></button>
